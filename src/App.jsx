...
import { Plus, List, CheckSquare, BarChart2, Target } from 'lucide-react';
...
const safeStorageSet = (key, value) => localStorage.setItem(key, value);
...
export default function App() {
  ...
  useEffect(() => {
    const result = safeStorageGet(STORAGE_KEY);
    if (result?.value) {
  ...
  const commitTask = (full) => setTasks(prev => prev.map(t => t.id === full.id ? full : t));
